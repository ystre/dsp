...
LOG_CLIENT="${G_ARTIFACT_DIR}/kafka-consumer-ft.log"
...
function stage-entry() {
    ...
    SPDLOG_LEVEL=trace "${BUILD_DIR,,}/src/tools/kafka-client" consume --broker "${broker}" --topic "${topic_name}" --group-id ft > "${LOG_CLIENT}" 2>&1&
    ...
    result=$(grep "Message consumed: " "${LOG_CLIENT}")

    check "$(sed --quiet '1p' <<< "${result}")" "key=keyA value=hello kafka" "First message (key and payload)"
    check "$(sed --quiet '2p' <<< "${result}")" "key=keyB value=hello again" "Second message (key and payload)"
